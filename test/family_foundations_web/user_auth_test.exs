defmodule FamilyFoundationsWeb.UserAuthTest do
  use FamilyFoundationsWeb.ConnCase, async: true

  alias Phoenix.LiveView
  alias FamilyFoundations.Accounts
  alias FamilyFoundations.Accounts.Scope
  alias FamilyFoundationsWeb.UserAuth

  import FamilyFoundations.AccountsFixtures
  ...
  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, FamilyFoundationsWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})
  ...
  describe "logout_user/1" do
    ...
    test "broadcasts to the given live_socket_id", %{conn: conn} do
      ...
      FamilyFoundationsWeb.Endpoint.subscribe(live_socket_id)
  ...
  describe "disconnect_sessions/1" do
    test "broadcasts disconnect messages for each token" do
      ...
      for %{token: token} <- tokens do
        FamilyFoundationsWeb.Endpoint.subscribe("users_sessions:#{Base.url_encode64(token)}")
      end
